...
public class LineageServiceTest {
  ...
  public void testBranchLineage() throws Exception {
    ...
    // Lineage for D3 should be D1 -> P1 -> D2 -> P2 -> D3
    //                                |
    //                                |
    //                           D0 ->|
    //
    lineage = lineageService.computeLineage(dataset3, 500, 20000, 100);
    Assert.assertNotNull(lineage);

    Assert.assertEquals(ImmutableSet.of(program1, program2), lineage.getPrograms());
    Assert.assertEquals(ImmutableSet.of(dataset0, dataset1, dataset2, dataset3), lineage.getData());
