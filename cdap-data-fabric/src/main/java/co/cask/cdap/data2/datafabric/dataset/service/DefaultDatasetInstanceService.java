 ...
 */
public class DefaultDatasetInstanceService implements DatasetInstanceService {
  private static final Logger LOG = LoggerFactory.getLogger(DefaultDatasetInstanceService.class);

  private final DatasetTypeService implManager;
  ...
  @Inject
  public DefaultDatasetInstanceService(DatasetTypeService implManager, DatasetInstanceManager instanceManager,
                                       DatasetOpExecutor opExecutorClient, ExploreFacade exploreFacade,
                                       CConfiguration conf,
                                       TransactionExecutorFactory txFactory,
                                       DatasetFramework defaultFramework,
                                       // @Nullable for unit tests -- used to perform dataset operations without RPCs
                                       @Nullable SystemDatasetFramework systemFramework) {
    DatasetFramework framework = systemFramework != null ? systemFramework : defaultFramework;
    this.opExecutorClient = opExecutorClient;
    this.implManager = implManager;
    this.instanceManager = instanceManager;
    this.exploreFacade = exploreFacade;
    this.usageRegistry = new UsageRegistry(txFactory, framework);
    this.namespaces = new DefaultNamespaceStore(txFactory, framework);
   ...
   */
  public void delete(Id.DatasetInstance instance) throws Exception {
    // Throws NamespaceNotFoundException if the namespace does not exist
    ensureNamespaceExists(instance.getNamespace());
   ...
   */
  private void deleteDataset(Id.DatasetInstance instance, DatasetSpecification spec) throws Exception {
    disableExplore(instance);
  ...
  private void ensureNamespaceExists(Id.Namespace namespace) throws Exception {
    if (!Id.Namespace.SYSTEM.equals(namespace) && namespaces.getNamespace(namespace) == null) {
